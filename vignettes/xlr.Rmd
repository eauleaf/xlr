...
*Package is currently somewhere between dev/beta.*
...
data professionals to work in both. However, working in both can be 
frustrating since it's time expensive to move data back forth between the
two. **xlr** is an effort to reduce the friction that comes from working in both 
spreadsheets and R.
...
data objects in a list, the `openxlsx` workbook object in case you want to change 
the workbook, or the workbook savepath as a string.
