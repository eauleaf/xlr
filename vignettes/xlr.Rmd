...
This package is somewhere between *dev and beta.*
...
want a series of decent-looking tables pushed to a workbook. **xlr** is
an effort to reduce the friction that comes from having to work in both
spreadsheets and R.
...
buried at various depths, `xl()` flattens and auto-names the workbook
...
```         
xl(your_datasets = enlist(flowers = iris, mtcars, datasets::USArrests)) 
...
`flower_data.xlsx` within your working directory. (Parameter `.path`
uses `here::here()` internally.)

`xl()` creates tibbles of all datasets you pass in, and if specified by
the user parameter `.return`, optionally returns (invisibly) the
...
The next video below is an example of copying file paths from a file
...
small to deal with saving and then retrieving later from a serialized
file or database. Instead, I just want to save the information as text
...
You can also highlight an expression in an editor window and then press
key-chord `ctrl + alt + shift + n`. Expressions evaluate to a deparsed
script that gets copied to the clipboard.
...
`repeated()` should probably be in dplyr. I use `repeated()` when
filtering a dataframe for identical observations, like:
...
`sys_open()` opens .xlsx files, but also opens any file, folder, or URL
since it requests the operating system open the location via the OS
commmand line interface. This function passes a path string (or many, if
you like) to your OS, asking it to open up the location using the
default application. The reason I use it instead of other functions is
because I've had better luck getting things to open when asking the user
system to make the decisions about how to open a file.
...
type; then double-click on the file to make sure it opens in that
default application.
...
to each. For example, consider the ridiculous list structure below and
list_iron's simplification of it:
...
*'rediculous\|flower_data'*. You can control that naming with
`name_spec` and `name_repair`. You can also apply a function while
flattening all of the leaves in the lists with the parameter `.f`, as
shown below:
...
`entibble()` automatically includes rownames, if they exist, as the
first column of the tibble. Also, `entibble()` does not repeat input
elements to obtain common lengths in order to produce a dataframe, and
`entibble()` treats a list of data inputs equivalent to comma-separated
data inputs.
