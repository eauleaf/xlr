...
<br>
*Package is somewhere between dev and beta.*
<br>
...
reader-friendly format and still the workhorse of the business community, but
spreadsheets don't have the data processing power of R, so its common for 
...
two, and most of the time, I just want a series of decent-looking tables pushed to a workbook.
...
`repeated()` should be in dplyr. I use `repeated()` when filtering 
a dataframe for identical observations, like:
...
`sys_open()` opens .xlsx files, but also any file or folder since it requests 
the operating system open it via the OS commmand line interface. This function passes a
path string (or many, if you like) to your OS, asking it to open up the location 
using the default application. The reason I use it instead of other functions is 
because I've had better luck getting things opened up when asking the system to 
make the decisions about how and with up to open the file. The function also 
opens URLs.
