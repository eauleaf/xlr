...
![](www/xlr.png){width="120"}

## ***R + Spreadsheets***

The p*ackage is somewhere between dev and beta.*
...
**xlr** makes it easier to move information between **spreadsheets** and
**R**.

Spreadsheets are a great tool for presenting 2-dimensional data in a
reader-friendly format and still the workhorse of the business
...
However, working in both can be frustrating. It's time expensive to read
data back forth between the two, and most of the time, I just want a
series of decent-looking tables pushed to a workbook. **xlr** is an
effort to reduce the friction that comes from having to work in both
spreadsheets and R.
...
buried at various depths, `xl()` will flatten and auto-name the workbook
and the spreadsheet tabs. Nested lists can be useful for the naming of
groups on spreadsheet tabs.
...
data objects in a list, or the `openxlsx` workbook object in case you
want to change things in the workbook, or the workbook savepath as a
string.
...
`enlist()` is for easy list naming. This function produces named lists,
...
careful about indexing by name. R returns only the first observation it
finds in the list.

-- Enlist has input parameter `.label`, that takes a naming function.
Internally, `.label` is passed to `nm` in `rlang::set_names()` and so
...
make sure to install a clipboard tool from a terminal:
...
that will recreate the data object from script. The script is a
formatted version of R's deparsed internal data representation.
...
The video below is an example of copying file paths and then using
...
small to deal with saving and retrieving from a serialized file or
database. Instead, I just want to save the information as text within
...
It's also convenient to use the quick keys for `enscript()`, which are
`ctrl + alt + shift + n`. (I remember the key-chord as all three control
keys, plus the `n`, which is short for enscript.)

To use the `enscript()` quick keys, Run the function,
`set_xlr_key_chords()` if you haven't already done so. Then:

1)  Then type a variable name like `mtcars` in your RStudio editor
...
`repeated()` should be in dplyr. I use `repeated()` when filtering a
dataframe for identical observations, like:
...
`sys_open()` opens .xlsx files, but also any file or folder since it
requests the operating system open it via the OS commmand line
interface. This function passes a path string (or many, if you like) to
your OS, asking it to open up the location using the default
application. The reason I use it instead of other functions is because
I've had better luck getting things opened up when asking the system to
make the decisions about how and with up to open the file. The function
also opens URLs.

If you aren't able to open a file with `sys_open()`, using your
...
type, and that double-clicking the file opens it in that default
application.
...
*'rediculous\|flower_data'*; you can control that naming. You can also
apply a function while flattening all of the leaves in the lists with
the parameter `.f`, as shown below:
...
order to produce a dataframe, and it treats a list of data inputs
equivalent to comma-separated data inputs.
