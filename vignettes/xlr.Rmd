...
run this function once in RStudio.) Then:
...
3) Then press `ctrl + alt + shift + c` all together. (It's just like copying with 
...
for `and` and `or` statements like this: 

```{r repeated-example2, eval = T}
# Identify any data repeated in the mpg or in the cyl columns
mtcars |> dplyr::filter(repeated(mpg) | repeated(cyl))
```
...
has a default application assigned that file type, and that double-clicking opens
it in that default application.

The video below is an example of opening up some PDF files by piping them from 
`paste_from_xl()` to `sys_open()`. 

![paste_from_xl() |> sys_open()](Screencast-sys_open.webm)
