...
xl(mtcars, iris, letters, AirPassengers)
```
...
buried somewhere. `xl()` recursively flattens and auto-names the spreadsheets and workbook. In fact, nested lists can be useful in the naming of
groupings within spreadsheet tabs if you provide a sheet-naming specification like `.tabname_spec = list(name_spec = "{outer}|{inner}")`.
...
You can also pass in an expression by piping 'enscript()' on the end
of anything, like:
...
It's also convenient to use quick keys for `enscript()`, which are
...
since it requests the operating system to open the input location via an OS
...
because I've had better luck getting things to open when asking the 
...
ridiculous <- list(list(list(list(flower_data = iris)), car_data = mtcars))
...
*'ridiculous\|flower_data'*. Control the naming with
...
`entibble()` makes a tibble with defaults useful for spreadsheets, i.e. it: 
- automatically includes rownames, if they exist, as the first column of the tibble
- does not repeat input elements to obtain common lengths in order to produce a dataframe
- treats a list of data inputs equivalent to comma-separated data inputs.
