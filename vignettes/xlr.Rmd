...
raw horsepower of R, so its helpful to use R for the things it's good at, 
and Excel/Libreoffice for the things they're good at. **xlr** is an effort 
to reduce the friction that comes from working in both. 



## Functions:

-   **`xlr()`** -- quickly output a list of datasets as spreadsheets in a
    workbook
...
`xlr()` is the primary function of the package 'xlr', and many
...
that `xlr()` is for quickly viewing datasets. 

It's easiest to see what `xlr()` does by passing it some example data;
try sending it some preloaded datasets, like:
...
To use `xlr()`, pass a dataframe or lists of dataframes. It does not
